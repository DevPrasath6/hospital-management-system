...
import { patientDoctors, patientRecords } from './PatientDashboard/patientDashboardData';
        ...
        <section className="doctor-dashboard">
          ...
          <section className="doctor-overview-grid">
            ...
            <article className="admin-card">
              ...
              <ul className="admin-compact-list">{patientDoctors.map((doctor) => <li key={doctor.name}>{doctor.name} - {doctor.specialty}</li>)}</ul>
            </article>
            <article className="admin-card">
              <div className="admin-card-heading"><h3>Recent records</h3><Link to="/patient-dashboard/records">Open section</Link></div>
              <ul className="admin-compact-list">{patientRecords.map((record) => <li key={record.record}>{record.record} - {record.status}</li>)}</ul>
            </article>
          </section>
        </section>
      </div>
